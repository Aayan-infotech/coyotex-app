...
//   void startTripTimer(int totalMinutes) {
...
//     _etaUpdateTimer = Timer.periodic(const Duration(seconds: 20), (timer) {
//       _recalculateETA();
//     });
...
//     if (_tripStartTime == null || routePolylinePoints.isEmpty) return;

//     // 1. Find closest point on the route to current position
//     int closestIndex = _findClosestPointIndex(
//       LatLng(currentLocationMarker.position.latitude,
//           currentLocationMarker.position.longitude),
//     );

//     // 2. Calculate remaining distance using cumulative distances
//     double distanceTraveled = cumulativeDistances[closestIndex];
//     double remainingDistance = totalRouteDistance - distanceTraveled;

//     // 3. Get current speed (convert m/s to km/h)
//     double speedKmph = (speed).clamp(1.0, 130.0); // Ensure realistic speed

//     // 4. Calculate remaining travel time (hours)
//     double remainingTravelTimeHours = remainingDistance / 1000 / speedKmph;
//     int remainingTravelMinutes = (remainingTravelTimeHours * 60).round();

//     // 5. Calculate remaining stop time
//     int remainingStopTime = 0;
//     if (currentMarkerIndex < selectedTripModel.markers.length) {
//       remainingStopTime = selectedTripModel.markers
//           .sublist(currentMarkerIndex)
//           .fold(0, (sum, marker) => sum + marker.duration);
//     }

//     // 6. Total remaining time
//     int totalRemainingMinutes = remainingTravelMinutes + remainingStopTime;

//     // 7. Update ETA
//     DateTime newEstimate =
//         DateTime.now().add(Duration(minutes: totalRemainingMinutes));

//     // 8. Compare with original estimate and notify if needed
//     if (originalEstimate != null) {
//       final diff = newEstimate.difference(originalEstimate!);
//       if (diff.inMinutes.abs() >= 20) {
...
//   //   final diff = originalEstimate!.difference(newEst);
//   //   if (diff.inMinutes.abs() >= 20) {
//   //     _sendChangeNotification(diff);
//   //     currentEstimate = newEst;
//   //   } else {
...
//   //     // Convert to double first, then round to nearest integer
//   //     double totalMinutes = double.parse(totalTravilMinutsWithStop.toString());
//   //     int roundedMinutes = totalMinutes.round();

//   //     _estimatedCompletionTime = _tripStartTime!.add(
//   //       Duration(minutes: roundedMinutes),
//   //     );

//   //     print(_estimatedCompletionTime);
//   //     notifyListeners();
//   //   } catch (e) {
//   //     print('Error in startTripTimer: $e');
//   //   }
//   // }
...
//   // void onMapStopCreated(GoogleMapController controller) {
//   //   mapController = controller;
...
//   //     destinationLatLng = tripModel.markers.last.position;
//   //     destination =
//   //         "${destinationLatLng.latitude},${destinationLatLng.longitude}";
//   //     waypointStrings = tripModel.markers
//   //         .sublist(0, tripModel.markers.length - 1)
...
//   //   // Build final URL without origin parameter
//   //   String finalUrl =
//   //       "$baseUrl&destination=$destination$waypointsParam&travelmode=driving&dir_action=navigate";
...
//   // }
//   String convertMinutesToHours(double distance) {
//     stopTime = 0;
//     for (var item in selectedTripModel.markers) {
...
//     // Initialize original ETA based on expected speed
//     double initialSpeedKmh = 40; // Default speed
//     double initialTravelTimeHours = totalRouteDistance / 1000 / initialSpeedKmh;
//     int initialTravelMinutes = (initialTravelTimeHours * 60).round();

//     originalEstimate =
//         DateTime.now().add(Duration(minutes: initialTravelMinutes + stopTime));
//     currentEstimate = originalEstimate;
...
//       startTripTimer(totalTravilMinutsWithStop.round());
...
//           TextEditingController 1stController = TextEditingController();
//           destinationControllers.add((1stController));
//           // pointA = latAndLng;
//           // markerId = 'start';
//         } else {
